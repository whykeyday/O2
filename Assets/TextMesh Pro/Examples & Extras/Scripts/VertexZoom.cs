        ...
        IEnumerator AnimateVertexColors()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < textInfo.meshInfo.Length; i++)
                {
                    ...
                    // Convert Vector4[] to Vector2[] for Mesh.uv
                    Vector4[] uv4s = textInfo.meshInfo[i].uvs0;
                    Vector2[] uv2s = new Vector2[uv4s.Length];
                    for (int j = 0; j < uv4s.Length; j++)
                        uv2s[j] = new Vector2(uv4s[j].x, uv4s[j].y);
